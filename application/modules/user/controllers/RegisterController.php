...
//         switch(Zend_Registry::get('enableRegistration')) {
//             case false :
//                 //$this->forward('index', 'page', 'default', null);
//                 break;
//             default:
//                 break;
//         }
                ...
                if (isset($values['userName']) && isset($values['passWord'])) 
                {
                	if($this->validator->isValid($values['userName'])) {
                	   ...
                	   // echo $date;
                		...
                		$link = $this->_request->getScheme() .'://'.$this->_request->getHttpHost().'/user/register/verify/' . $id . '/' . $timestamp;
                		...
                		$this->_helper->flashMessenger->addMessage('Thank you for registering, you will receive a verification email shortly. Please check your spam folder.');
                		$this->forward('success', 'register', 'user');
                		
                	} else {
                		// we need to notify them they need to choose another userName since that one is in use
                		$this->_helper->flashMessenger->addMessage('UserName already in use!!');
                    ...
                    if($row->uStatus === 'disabled') 
                    {
                        ...
                        if( isset($row->userId) && $row->userId > 0 && $row->userId !== null && $row->uStatus === 'disabled')
                        {
                            if( ($this->_request->uid === $row->userId) && ( $storedHash === $requestHash) )
                            {
                        		...
                        		{
                        		    $this->_helper->flashMessenger->addMessage('Your account has been verified. You can now login!');
                        		}
                            } else {
                        ...
                        $this->_helper->flashMessenger->addMessage('Your account has already been enabled');
                    }
            } else {
                $this->_helper->flashMessenger->addMessage('Your account has been deleted by an administrator.');
            }
        } 
